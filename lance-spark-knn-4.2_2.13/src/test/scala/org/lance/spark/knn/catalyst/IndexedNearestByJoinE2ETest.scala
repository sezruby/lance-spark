...
/**
 * End-to-end SQL test for the Catalyst integration. Drives the full path:
 *
 ...
 *
 * Coverage:
 *   - SQL `APPROX NEAREST k BY DISTANCE vector_l2_distance(...)` parses, the rule rewrites to a
 *     single `LanceKnnJoinLogicalPlan`, the strategy lowers it to `LanceKnnJoinExec`, which
 *     executes against a real Lance dataset; results match the brute-force oracle. With no vector
 *     index built, Lance does an exact per-fragment scan, so any disagreement is a bug.
 *   - Right-side `WHERE` round-trips through the prefilter pushdown.
 *   - With the gating config disabled, the same SQL falls through to Spark's
 *     `RewriteNearestByJoin` (cross-product + `MaxMinByK`) — proves the rule's opt-in behavior.
 */
class IndexedNearestByJoinE2ETest {
   ...
   * AND the result must match the brute-force oracle on every left row.
   */
  @Test def testSqlApproxNearestRoutesThroughIndexedPathAndMatchesOracle(): Unit = {
    ...
    val joinLogicals = optimized.collect { case p: LanceKnnJoinLogicalPlan => p }
    assertTrue(
   ...
   * `LanceKnnJoinLogicalPlan` and (importantly) results still match the oracle. This proves
