 ...
 * [[LanceKnnJoinStrategy]] then lowers that node to [[LanceKnnJoinExec]], which drives the same
 * no-shuffle `LanceKnnJoinStage.runPartition` the DataFrame API path uses.
 *
