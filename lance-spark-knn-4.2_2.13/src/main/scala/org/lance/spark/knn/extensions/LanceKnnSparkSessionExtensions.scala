...
class LanceKnnSparkSessionExtensions extends (SparkSessionExtensions => Unit) {
  override def apply(extensions: SparkSessionExtensions): Unit = {
    ...
    // Lowers the single `LanceKnnJoinLogicalPlan` the rule emits to `LanceKnnJoinExec` — the same
    // no-shuffle `LanceKnnJoinStage.runPartition` the DataFrame API path drives.
