 ...
 * with `|R|`, which is what `LanceKnnSizeGate` gates on the DataFrame path.
 *
 * Both the DataFrame API ([[org.lance.spark.knn.IndexedNearestJoin]]) and the SQL Catalyst node
 * (`LanceKnnJoinExec` in the 4.2 module) drive this same `runPartition`, so probe/trim/materialize
